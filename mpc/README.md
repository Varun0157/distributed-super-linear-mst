...
In order to automatically generate a large graph, generate a ground truth using a sequentia Kruskal's algorithm, generate the graph as predicted by _Filtering_, and compare the two, use the [test script](./scripts/test.sh).
...
- [ ] prevent loading of entire graph into memory - could store some graph metadata instead of loading to find it
